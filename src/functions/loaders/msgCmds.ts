  ...
  for (const category of categories) {
    ...
    for (const commandFile of commandFiles) {
      const fileUrl = pathToFileURL(
        resolve(__dirname, "../../commands", category, commandFile),
      ).href;
      const command = new (await import(fileUrl)).default() as AbstractCommand;

      command.category = category;
      command.name = commandFile.split(".")[0];
      command.aliases.push(commandFile.split(".")[0]);
